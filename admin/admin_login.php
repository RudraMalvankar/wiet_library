...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    // Validate credentials using the auth system
    $admin = validateAdmin($email, $password);
    
    if ($admin) {
        // Check if admin must change password on first login
        if (mustChangePassword($email)) {
            // Redirect to password change page
            $_SESSION['temp_email'] = $email;
            $_SESSION['must_change_password'] = true;
            header('Location: wiet_lib/admin/change-password.php');
            exit();
        }
        ...
        header('Location: wiet_lib/admin/layout.php');
        exit();
    } else {
...
<head>
    ...
    <title>Admin Login - WIET Library</title>
